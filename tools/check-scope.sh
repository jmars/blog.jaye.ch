...
# nothing else.
unexpected="$(find content -type f 2>/dev/null | sort | grep -vE '^content/(meditation-harm|meditation-harm-summary|anxiety-damping|what-actually-works|recovery-is-not-immunity|manufacturing-the-collapse|sacred-science|empty-leader|what-the-traditions-knew|safeguards|the-label)\.md$' || true)"
